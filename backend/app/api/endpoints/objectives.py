...
def get_objective_enums():
    return {
        ...
        "strategic_perspective": [
            e.value for e in objective_models.ObjectiveStrategicPerspective
        ],
...
def list_progress_updates_for_objective(
    ...
    return crud.crud_progress_update.get_progress_updates_by_objective(
        db, objective_id, skip=skip, limit=limit
    )


@router.post(
    "/{objective_id}/progress-updates",
...
def create_progress_update_for_objective(
    ...
    return crud.crud_progress_update.create_progress_update(
        db=db, obj_in=progress_update_in
    )
...
def update_progress_update(
    ...
    if not db_obj:
        raise HTTPException(status_code=404, detail="Progress update not found")
    return crud.crud_progress_update.update_progress_update(
        db=db, db_obj=db_obj, obj_in=progress_update_in
    )
...
def delete_progress_update(
    ...
    if not db_obj:
        raise HTTPException(status_code=404, detail="Progress update not found")
    return crud.crud_progress_update.delete_progress_update(
        db=db, progress_update_id=progress_update_id
    )
